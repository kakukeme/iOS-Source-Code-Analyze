...
@interface YYLabel() <YYTextDebugTarget, YYTextAsyncLayerDelegate> {
    ...
    struct {
        unsigned int layoutNeedUpdate : 1;      // 是否需要更新布局
        unsigned int showingHighlight : 1;      // 显示类似链接高亮
        
        unsigned int trackingTouch : 1;         // 触摸跟踪
        unsigned int swallowTouch : 1;          // 吞了
        unsigned int touchMoved : 1;            //
        
        unsigned int hasTapAction : 1;          // 点击动作
        unsigned int hasLongPressAction : 1;    // 长按动作
        
        unsigned int contentsNeedFade : 1;      // 内容是否支持逐渐消失
    } _state;
}
@end
...
- (YYTextAsyncLayerDisplayTask *)newAsyncDisplayTask {
    ...
    /**
     渲染时，首先获取YYTextLayout, 一般包含了 YYTextContainer 和  NSAttributedString 两部分，
     分别负责文本展示的形状和内容;
    ...
    task.display = ^(CGContextRef context, CGSize size, BOOL (^isCancelled)(void)) {
        ...
        if (verticalAlignment == YYTextVerticalAlignmentCenter) {
            ...
            if (drawLayout.container.isVerticalForm) {          // 垂直布局
                point.x = -(size.width - boundingSize.width);
            } else {
                point.y = (size.height - boundingSize.height);  // size 外围矩形； boundingSize文字区域；
            }
        }
