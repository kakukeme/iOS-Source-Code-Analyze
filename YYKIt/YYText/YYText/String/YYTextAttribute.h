...
// 文本垂直对齐方式，垂直方向text的位置,
/**
 Text vertical alignment.
 */
typedef NS_ENUM(NSInteger, YYTextVerticalAlignment) {
    YYTextVerticalAlignmentTop =    0, ///< Top alignment.      居顶
    YYTextVerticalAlignmentCenter = 1, ///< Center alignment.   居中
    YYTextVerticalAlignmentBottom = 2, ///< Bottom alignment.   居下
};
...
// YYText中定义的Attribute Name。主要是独有的一些类型。
...
/**
 YYTextShadow：用处和NSShadow一样，只是比NSShadow多了一些功能，比如说可以使用blendMode（图形混合模式）、可以在shadow上再加一层shadow。关于blendMode的学习，可以参见喵神的博客：https://onevcat.com/2013/04/using-blending-in-ios/
...
/**
 ...
 YYTextDecoration：实现下划线（underline）和中间截线（strikethrough）时使用，线条的形式给出了几种样式，可以通过YYTextLineStyle枚举查看。具体是underline还是strikethrough是在NSAttributedString+YYText中NSMutableAttributeString（YYText）中实现的方法。
