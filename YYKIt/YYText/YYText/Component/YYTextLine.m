...
/**
 从CTLineRef中取出对应的行宽、上行高度、下行高度、行间距、range和第一个字型符的位置（这个在垂直布局会用到）。之后调用reloadBounds方法，重新计算当前行的bounds、attachments所在的range和rect
 
 */
- (void)setCTLine:(_Nonnull CTLineRef)CTLine {
    if (_CTLine != CTLine) {
        if (CTLine) CFRetain(CTLine);
        if (_CTLine) CFRelease(_CTLine);    // 新值retain，旧值release；
        _CTLine = CTLine;
        if (_CTLine) {
            
            // 从CTLineRef中取出对应的行宽、上行高度、下行高度、行间距、range
            ...
            if (CTLineGetGlyphCount(_CTLine) > 0) {
                CFArrayRef runs = CTLineGetGlyphRuns(_CTLine);  // 字型符CTRunRef
                CTRunRef run = CFArrayGetValueAtIndex(runs, 0);
                CGPoint pos;
                CTRunGetPositions(run, CFRangeMake(0, 1), &pos);
                
                _firstGlyphPos = pos.x;     // 第一个字型符的位置
            } else {
                _firstGlyphPos = 0;
            }
...
/**
 重新计算当前行的bounds、attachments所在的range和rect
 
 ？？？？？
 
 */
- (void)reloadBounds {
    if (_vertical) {    // 竖直方向；相当于下面水平方向竖起来；
        ...
        /**
         position为baseline的位置；
         (_position.x, _position.y - _ascent) : 当前行左上角；
         _lineWidth: 行宽
         _ascent + _descent: 行高； 上行高度(Ascent)加下行高度(Decent)就是行高
         */
    ...
    NSUInteger runCount = CFArrayGetCount(runs);        // CTRunRef字型 字符个数
