...
/**
 head、foot 记录每行头、尾位置；水平的判断y；垂直的判断x;
...
// CGColor 转为 UIColor；iOS7 CoreText的bug
/**
...
- (void)modifyLines:(NSArray *)lines fromText:(NSAttributedString *)text inContainer:(YYTextContainer *)container {
    if (container.verticalForm) {
        ...
        for (NSUInteger i = 0, max = lines.count; i < max; i++) {
            ...
            // 多行时，计算每行的y值；垂直时计算x值；_fixedLineHeight固定行高
...
@property (nonatomic, assign) YYRowEdge *lineRowsEdge; ///< top-left origin  原点左上角
...
+ (YYTextLayout *)layoutWithContainer:(YYTextContainer *)container text:(NSAttributedString *)text range:(NSRange)range {
    ...
    // 3、CoreText bug 当绘制加入的表情emoji时，从iOS 8.3开始
    ...
    // 需要使用更大的约束尺寸，在iOS10中使用 CTFramesetterCreateFrame 创建 CTFrame时
    ...
    // 4、判断是否设置了path属性和exclusionPaths数组，做相应的计算拿到cgpath，
    // 如果cgpath为空则goto fail 返回nil（如果设置了path，size和insets就没有用了）
    
    // 根据 YYTextContainer 生成 CGPath;
    // set cgPath and cgPathBox
    if (container.path == nil && container.exclusionPaths.count == 0) {     // 不存在path、exclusionPaths
        ...
        // 坐标系转换，参考这里http://www.jianshu.com/p/b6e7ff3c47f6
        // 存在path, 且path是个矩形；但是exclusionPaths不存在
        CGRect rect = CGRectApplyAffineTransform(cgPathBox, CGAffineTransformMakeScale(1, -1)); // 坐标系，左下角->左上角
        cgPath = CGPathCreateWithRect(rect, NULL); // let CGPathIsRect() returns true
    } else {
        rowMaySeparated = YES;
        CGMutablePathRef path = NULL;
        if (container.path) {       // 有path
            path = CGPathCreateMutableCopy(container.path.CGPath);
        } else {    // 没有path，使用size、insets构建rect
            CGRect rect = (CGRect) {CGPointZero, container.size };
            rect = UIEdgeInsetsInsetRect(rect, container.insets);
        ...
        if (path) {
            ...
            [layout.container.exclusionPaths enumerateObjectsUsingBlock: ^(UIBezierPath *onePath, NSUInteger idx, BOOL *stop) {
                CGPathAddPath(path, NULL, onePath.CGPath); // 添加排除路径
            }];
            
            cgPathBox = CGPathGetPathBoundingBox(path);
            CGAffineTransform trans = CGAffineTransformMakeScale(1, -1);  // 翻转坐标系
    ...
    if (lineCount > 0) {
        ...
        CTFrameGetLineOrigins(ctFrame, CFRangeMake(0, lineCount), lineOrigins); // 每一行的origins
    }
    ...
    // calculate line frame     计算每一行CTLine的frame，
    NSUInteger lineCurrentIdx = 0;
    for (NSUInteger i = 0; i < lineCount; i++) {
        CTLineRef ctLine = CFArrayGetValueAtIndex(ctLines, i);      // CTLine
        CFArrayRef ctRuns = CTLineGetGlyphRuns(ctLine);             // CTRuns
        if (!ctRuns || CFArrayGetCount(ctRuns) == 0) continue;
        
        // CoreText coordinate system       CoreText坐标系，右手系，原点左下角
        CGPoint ctLineOrigin = lineOrigins[i];
        
        // UIKit coordinate system      UIKit坐标系，左手系，原点左上角
        CGPoint position;
        position.x = cgPathBox.origin.x + ctLineOrigin.x;
        position.y = cgPathBox.size.height + cgPathBox.origin.y - ctLineOrigin.y;   // why???
        ...
        if (newRow) rowIdx++;
        lastRect = rect;
        lastPosition = position;    // 保存上次rect、position
        
        line.index = lineCurrentIdx;
        line.row = rowIdx;
        [lines addObject:line];     // 添加到lines
        rowCount = rowIdx + 1;
        lineCurrentIdx ++;
        ...
        if (i == 0) textBoundingRect = rect;
            ...
            if (maximumNumberOfRows == 0 || rowIdx < maximumNumberOfRows) {
                textBoundingRect = CGRectUnion(textBoundingRect, rect);     // 随着遍历每行，增大文本区域，
            }
        }
    }// for end
    ...
    if (rowCount > 0) {
        if (maximumNumberOfRows > 0) {
            if (rowCount > maximumNumberOfRows) { // 大于最大行数，需要截断
                needTruncation = YES;
        ...
        // 实现了linePositionModifier这个协议，有的话调用协议方法;
        // 让用户有机会修改行的位置。
        ...
        CGFloat lastHead = 0;
        CGFloat lastFoot = 0;   // head、foot 记录每行头、尾位置；水平的判断y；垂直的判断x;
        for (NSUInteger i = 0, max = lines.count; i < max; i++) {
            YYTextLine *line = lines[i];
            CGRect rect = line.bounds;
    ...
    { // calculate bounding size
        CGRect rect = textBoundingRect;     // textBoundingRect 显示文本区域
        if (container.path) {
