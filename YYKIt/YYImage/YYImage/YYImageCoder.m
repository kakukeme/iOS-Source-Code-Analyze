...
#pragma mark - 根据index，得到图片解码后的frame
/**
 *  通过最初的图像解码得到指定Index的frame，
 *
 *  图像解码后，放到_frames中，参考_updateSourceImageIO；
 */
...
- (void)_updateSourceImageIO {
    ...
    // 图像宽，高，显示方向初始化, 循环次数; _loopCount = 0 代表无限
    _width = 0;
    _height = 0;
    ...
    // 1、处理图像源
    // 使用CGImageSourceCreateWithData来通过 data 创建 CGImage 对象，
    // 如果是增量的 data 则使用CGImageSourceCreateIncremental和CGImageSourceUpdateData来生成 CGImage
    if (!_source) {
        if (_finalized) {
    ...
    for (NSUInteger i = 0; i < _frameCount; i++) {
        ...
        [frames addObject:frame];   // 通过最初的图像解码得到指定Index的frame
