...
- (nullable NSData *)sd_imageDataAsFormat:(SDImageFormat)imageFormat {
    ...
    if (self) {
        ...
        if (imageFormat != SDImageFormatUndefined) {
            usePNG = (imageFormat == SDImageFormatPNG);  // 是否是PNG类型的图片
        }
        
        // 根据不同的图片类型获取到对应的图片data
        if (usePNG) {
            imageData = UIImagePNGRepresentation(self); // png
        } else {
            imageData = UIImageJPEGRepresentation(self, (CGFloat)1.0);  // jpeg
        }
#else
